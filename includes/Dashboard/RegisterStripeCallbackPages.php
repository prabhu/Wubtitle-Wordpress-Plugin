...
class RegisterStripeCallbackPages {
	...
	public function register_success_page() {
		$custom_hook = add_submenu_page( '', 'page_payment_success', 'page_payment_success', 'manage_options', 'stripe-success-payment', function(){} );
		add_action( "load-$custom_hook", array( $this, 'render_success_payment_page' ) );
		$custom_hook = add_submenu_page( '', 'page_update_success', 'page_update_success', 'manage_options', 'stripe-success-update', function(){} );
		add_action( "load-$custom_hook", array( $this, 'render_success_update_page' ) );
		$custom_hook = add_submenu_page( '', 'page_payment_cancel', 'page_payment_cancel', 'manage_options', 'stripe-cancel-payment', function(){} );
		add_action( "load-$custom_hook", array( $this, 'render_cancel_payment_page' ) );
		$custom_hook = add_submenu_page( '', 'page_update_cancel', 'page_update_cancel', 'manage_options', 'stripe-cancel-update', function(){} );
